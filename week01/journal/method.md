1.1 方法的概念
  ...
  方法创建后并不是直接可以运行的，需要手动使用后，才执行，该过程为方法调用
...
3.1 带参数方法定义和调用
  ...
  注意：方法定义时，参数中的数据类型与变量名都不能缺少，缺少任意一个程序将报错。方法定义时，多个参数之间使用逗号分隔
...
5.2 方法的通用格式
  ...
  ​如果方法操作完毕，没有数据返回，写void，而且方法体中一般不写return
  方法名：调用方法时候使用的标识
...
7.1 数组遍历
  ...
  思路：
  ①因为要求结果在一行上输出，所以这里需要一个新的输出语句System.out.print(“内容”);
